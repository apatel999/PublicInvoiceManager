...
namespace InvoiceManagerWeb.Print
{
    public class InvoiceTextReport
    {
        ...
        private string Header()
        {
            ...
            template = this.replaceTemplateText(template, "{InvoiceNo}", value, true);

            value = this.order != null ? this.order?.CreateDate.ToString("dd/MM/yyyy") : "";
            template = this.replaceTemplateText(template, "{Date    }", value, true);

            value = !string.IsNullOrEmpty(this.order?.StoreInfo?.BillingInformation?.Name) ? this.order?.StoreInfo?.BillingInformation?.Name : "";
            template = this.replaceTemplateText(template, "{                           CustomerName}", value, true);
